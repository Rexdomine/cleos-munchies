...
function customerText(details, lines, total, reference) {
  return `Cleo's Munchies preorder ${reference}\n\nThanks for your order, ${details.name}.\n\n${textItems(lines)}\n\nFood subtotal: ${pounds(total)}\n\nYour food will be ready within 48 hours. Delivery arrangements and any delivery charge will be confirmed separately. Payment is not confirmed until your Monzo payment is manually matched.\n\nOpen Monzo: ${MONZO_PAYMENT_URL}\nUse ${reference} in the payment Notes.`;
}
...
export default async function handler(req, res) {
  ...
  const customerHtml = { html: emailShell({ preheader: `Your Cleo’s Munchies preorder ${reference} is safely with us.`, eyebrow: 'Order received', title: `Thanks, ${details.name}`, intro: `We’ve received your preorder and sent the details to the Cleo’s Munchies kitchen team.`, reference, content: `${orderCard(lines, total, details)}<div style="height:18px;line-height:18px">&nbsp;</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0"><tr><td style="padding:16px 18px;background:#f3e4d8;border-left:4px solid #bd392e;border-radius:8px"><p style="margin:0;color:#5f4037;font-size:14px;line-height:1.55"><strong style="color:#bd392e">Payment is not confirmed yet.</strong><br>Your food will be ready within 48 hours. Delivery arrangements and any delivery charge will be confirmed separately.</p></td></tr></table><div style="height:22px;line-height:22px">&nbsp;</div><a href="${MONZO_PAYMENT_URL}" style="display:inline-block;padding:14px 20px;background:#bd392e;border-radius:7px;color:#ffffff;font-size:14px;font-weight:bold;text-decoration:none">Continue to Monzo&nbsp; →</a><p style="margin:12px 0 0;color:#766e65;font-size:12px;line-height:1.5">Put <strong>${escapeHtml(reference)}</strong> in the payment Notes so we can match your order manually.</p>`, footer: 'Questions? Reply to this email and the Cleo’s Munchies team will help.' }), textContent: customerText(details, lines, total, reference) };
