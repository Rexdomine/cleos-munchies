...
- 2026-09-10 okra correction: owner reported irrelevant okra across the generated menu images. A category-by-category visual audit found 25 affected non-okra assets and 34 clean non-okra assets; Seafood Okra and Okra Soup are intentionally exempt. Surgical GPT Image 2 edits are in progress, with exact dish identity and paths preserved.
- Okra-clean candidate gate: 25/25 affected source edits passed full-resolution inspection and all eight affected category screenshots passed card-size inspection. The exact Git delta contains only the intended 25 WebPs; Seafood Okra and Okra Soup hashes are unchanged. Manifest remains 61/61 with 61 unique hashes. `npm test` passed 25/25, production build passed, `npm audit --audit-level=high` found 0 vulnerabilities, full Chromium passed 20/20, `git diff --check` passed, and the secret scan is clean.
- Okra-clean release: commit `0ac35f54d1e6cd33edfd3f0ff6e3b187523955c6` was pushed to GitHub `master`; Vercel deployment `dpl_5Gw7HQ6CnBi3V1oA1z8n4wyY6xhR` reported production `READY` for that exact SHA. Seven representative hosted assets returned HTTP 200 and matched their manifest SHA-256 values byte-for-byte, including five corrected categories and both preserved okra soups.
...
- Mobile visual release: GitHub `master` and the exact Vercel production deployment matched the tested commit. Canonical HTML, CSS, and JavaScript returned HTTP 200; hosted CSS contained `button{cursor:pointer;color:inherit}`, hosted JavaScript contained `energy-burst`, and the Unicode `✳` glyph was absent.
